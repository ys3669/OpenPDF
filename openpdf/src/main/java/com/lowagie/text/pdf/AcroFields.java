...
public class AcroFields {
  ...
  /**
   * Sets the option list for fields of type list or combo. One of <CODE>exportValues</CODE> or <CODE>displayValues</CODE> may be
   * <CODE>null</CODE> but not both. This method will only set the list but will not set the value or appearance. For that, calling
   ...
   * @param name the fully qualified field name or the partial name in the case of XFA forms
   * @param value the field value
   * @param display the string that is used for the appearance. If <CODE>null</CODE> the <CODE>value</CODE> parameter will be used
   * @return <CODE>true</CODE> if the field was found and changed,
   * <CODE>false</CODE> otherwise
   * @throws IOException on error
   * @throws DocumentException on error
  ...
  /**
   * Gets the field box positions in the document. The return is an array of <CODE>float</CODE> multiple of 5. For each of this groups the
   ...
   * Acrobat. Don't use it unless your environment is well controlled. The default is <CODE>true</CODE>.
   *
   ...
   *
   * @param field the signature field name
   * @return an <CODE>InputStream</CODE> covering the revision. Returns <CODE>null</CODE> if it's not a signature field
   * @throws IOException on error
   */
  ...
  /**
   * Creates a new pushbutton from an existing field. If there are several pushbuttons with the same name only the first one is used. This
   * pushbutton can be changed and be used to replace an existing one, with the same name or other name, as long is it is in the same
   * document. To replace an existing pushbutton call {@link #replacePushbuttonField(String, PdfFormField)}.
   *
  ...
  /**
   * Creates a new pushbutton from an existing field. This pushbutton can be changed and be used to replace an existing one, with the same
   * name or other name, as long is it is in the same document. To replace an existing pushbutton call {@link
  ...
  /**
   * Replaces the first field with a new pushbutton. The pushbutton can be created with {@link #getNewPushbuttonFromField(String)} from the
  ...
  /**
   * Replaces the designated field with a new pushbutton. The pushbutton can be created with {@link #getNewPushbuttonFromField(String, int)}
   * from the same document or it can be a generic PdfFormField of the type pushbutton.
   *
   * @param field the field name
   * @param button the <CODE>PdfFormField</CODE> representing the pushbutton
   * @param order the field order in fields with same name
